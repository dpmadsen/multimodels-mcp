...
import { fetchLmStudioModels, resolveLmStudioProvider } from "./catalog.js";
...
// O catalogo conserva endpoints iniciais configurados, mas nao pode seguir um
// redirect. Antes de alterar/remover, conferir requisito, diff, historico,
// MEMORY.md, plano e docs/test-change-log.md.
test("consulta ao catalogo LM Studio usa redirect manual", async () => {
  ...
  try {
    assert.deepEqual(await fetchLmStudioModels(config.providers.lmstudio as OpenAICompatProvider), { available: true, models: ["m"] });
