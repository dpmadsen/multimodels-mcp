...
- **"With hands" lanes** → GLM (z.ai), DeepSeek and Kimi (Moonshot) piloting a disposable headless Claude Code pointed at their Anthropic-compatible endpoint — plus **Claude itself on your subscription** (`claude-maos`, no API key). Every lane can read/search with `Read`/`Glob`/`Grep` and cannot edit; only the subscription lane can also run `npm test` / `npm run build`. API-key-backed lanes have no Bash tool.

The same pattern works for any MCP-capable agent — nothing here is Claude-specific except where it's registered.

Keyed Claude lanes also pass `--settings '{"disableAllHooks":true}'` to disable project/local hooks, which can execute commands independently of the model's tool list. This overrides project/local settings; administrator-managed policy still takes precedence. Subscription settings are unchanged. The local tests verify the spawned arguments; native hook suppression has not been run here because `claude` is unavailable.
...
### Security boundaries (current, verified)

- `workdir` is structurally optional because `delegate_task` serves every provider, but it is mandatory and canonicalized for Codex, Gemini, and every CLI/"with hands" lane. Direct API lanes ignore it. Approved providers may intentionally receive project code. Child processes receive only the environment needed by their lane, and unrelated credentials are excluded.
- API-key-backed Claude lanes expose only `Read`, `Glob`, and `Grep`: the CLI receives the restrictive `--tools` list plus non-interactive `dontAsk`, so project settings cannot restore Bash/Edit/Write. The subscription Claude lane deliberately keeps `Read`/`Glob`/`Grep` plus `npm test` / `npm run build`.
- Provider redirects are rejected. `contextTokens` is optional model metadata only; `maxOutputTokens` is the provider request hint (`modelLimits[model]` → provider → 32,000); `maxResponseBytes` is the locally enforced cap (`modelLimits[model]` → provider → `defaults` → 10 MiB) for API and CLI results.
- Panel mutations require the exact origin and host of the running panel (default `http://127.0.0.1:4747`) plus JSON content type. JSONL diagnostics go to stderr and never include prompts, source, responses, partials, URLs, redirect locations, authorization headers, or credentials.

These controls do not claim that MCP roots enforce filesystem permissions or that writable CLI modes have passed native confinement verification.
...
- **"With hands" lanes** — need the [Claude Code CLI](https://claude.com/claude-code) on your PATH; each lane is a `claude-cli` provider with write tools blocked. A lane with `baseUrl` + `envKey` is a third-party engine (that address, that key, run under a throwaway `CLAUDE_CONFIG_DIR`) and exposes only `Read`/`Glob`/`Grep`, enforced with `--tools` and `dontAsk`; it cannot invoke Bash/npm. A lane with neither field is a subscription lane, run under your real Claude Code login with no key, and keeps the two npm verification commands. One engine, two deliberately different tool sets. Shipped vendor lanes: `glm-maos:glm-5.2` (z.ai, `ZAI_API_KEY`), `deepseek-maos:deepseek-v4-pro` / `deepseek-maos:deepseek-v4-flash` (`https://api.deepseek.com/anthropic`, `DEEPSEEK_API_KEY`, pay-per-use) and `kimi-maos:kimi-k3` (`https://api.moonshot.ai/anthropic`, `MOONSHOT_API_KEY`). The panel shows a key field on these cards (last-4-only, as always).
- **`claude-maos` — Anthropic's own models, on your subscription.** Ids: `claude-maos:claude-fable-5`, `claude-maos:claude-opus-5`, `claude-maos:claude-opus-4-8`, `claude-maos:claude-sonnet-5`. Its tools are `Read`/`Glob`/`Grep` plus `npm test` / `npm run build`, with no Edit/Write. Unlike the keyed lanes, this subscription lane intentionally retains Bash only for those two npm commands. No key, so no key field in the panel. Three things worth knowing before you reach for it:
