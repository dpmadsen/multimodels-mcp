...
Two requested lanes on the same two stations: [`benchmark/rodada4-raias-novas/`](benchmark/rodada4-raias-novas/). **Kimi K3** (text-only, via OpenRouter) ran and became the **second text-only lane ever to beat the cutoff trap** — 14/14 on the zod v4 station from memory alone, joining Grok 4.5 in the "fresh memory" club. The official scoreboard (@5min) showed 3 of 6 runs dying, but a verification run exposed an unfair deadline: Kimi ran at OpenRouter's 5-minute cap while the z.ai lane got 15 minutes. **5 of 6 runs delivered perfect scores once the deadline was equalized** (15 min, matching z.ai — now the permanent default); what's left is **one systematic failure mode** (it reasons to completion but never emits the final answer, on the same cell 3×). It's the heaviest reasoner in the study (~$0.20 per delivered task, $3/$15 per M). The two **Gemini** lanes (3.1 Pro high and 3.6 Flash high) are **pending** — the Google subscription quota ran out; that window resets ~Jul 29.
